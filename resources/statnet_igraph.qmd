...
For igraph this is accomplished using `as_data_frame()` and indicating whether you want to convert to a vertex or edge dataframe using the `what=` argument. In the below example I calculate the degree and betweenness centrality of each node, then convert it to a dataframe and show the first 5 rows. 
...
A solution to this is to pre-end function calls with the library the come from: 
