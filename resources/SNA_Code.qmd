...
```{r}
#| echo: true
net_mat <- read.csv(here::here("resources/network_data/wheel_of_time_ep1.csv"), row.names=1)
...
```{r}
#| echo: true
mat <- read.csv(here::here("resources/network_data/colleges.csv"), row.names = 1)
mat <- as.matrix(mat)
...
#| echo: true
library(igraph)
net <- read_graph(here::here("resources/network_data/strike.paj"), format="pajek")
ecount(net)
vcount(net)
```
...
#| echo: true
#| out-height: 40%
dnc_net <- read_graph(here::here("resources/network_data/dnc.gml"), format='gml')
...
df <- read.csv(here::here("resources/network_data/COCAINE_MAMBO.csv"), row.names=1)
mat <- as.matrix(df)
...
```{r}
#| echo: true
net <- read_graph(here::here("resources/network_data/london.graphml"), format="graphml")
net
```
...
inc_df <- read.csv(here::here("resources/network_data/scotland.csv"), row.names=1)
...
diag(bonac_trans) <- 0
bonac_comp_net <- graph_from_adjacency_matrix(bonac_trans, 
    diag=F, mode="undirected", weight=T)
```
...
democracy_df <- read.csv(here::here("resources/network_data/cow/democracy_data.csv"))
```
...
```{r}
#| echo: false 
net <- read_graph(here::here("resources","network_data","french_office.graphml"), format="graphml")

ggraph(net, "kk") + geom_edge_link(color="gray") + 
...
#| output: false
set.seed(1)
mod_full <- ergm(asNetwork(net)~edges + nodematch("Department") + 
  nodefactor("Department") + 
  gwdegree(decay=0.25, fixed=TRUE) +
  gwesp(decay=0.25, fixed=TRUE)  )
```
...
mod <- ergm(net ~ edges +nodematch("Sexo") + nodematch("Grupo") + 
            nodefactor("Sexo") + nodefactor("Grupo") +
            nodecov("prosocial") + 
    gwesp(decay=0.5, fixed=T) + gwdegree(0.5, fixed=T))
est_gof <- gof(mod)
```
...
mod <- ergm(net ~ edges +nodematch("Sexo") + nodematch("Grupo") + 
            nodefactor("Sexo") + nodefactor("Grupo") +
            nodecov("prosocial") + 
    gwesp(decay=0.25, fixed=T) + gwdegree(0.25, fixed=T))
est_gof <- gof(mod)
```
