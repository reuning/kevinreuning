...
The `sna` package has a helper function for this `sna::gvectorize()`
...
sna::gvectorize(test_mat, mode="graph") ### for undirected

```

### Creating a Dataframe 

We can do the same thing to our distance matrix (first converting it to a normal matrix) and use `na.omit()` to drop the `NA`s

```{r}
#| echo: true 
data <- data.frame("Age_Dist"=sna::gvectorize(as.matrix(age_dist), mode="graph"),
                   "SE_Dist"=sna::gvectorize(as.matrix(london_se), mode="graph"))
data <- na.omit(data)
data[1:5,]
```
