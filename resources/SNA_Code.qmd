...
- [ca](https://cran.rstudio.org/web/packages/ca/index.html): For correspondence analysis. 
...
- `ca()` from the [ca](https://cran.rstudio.org/web/packages/ca/index.html) package. 
...
The [data here](network_data/colleges.csv) shows the number of a majors that graduated from colleges in Ohio in 2024.
...
#| out-height: 80%
#| echo: true
#| fig-alt: "Basic scatter plot showing dots on a Dim 1 and Dim 2 axis. The points are the majors."
...
#| out-height: 80%
#| echo: true
#| fig-alt: "Basic scatter plot showing dots on a Dim 1 and Dim 2 axis. The majors and schools are colored differently here."
...
#| layout-ncol: 2
#| echo: false
ggraph(net, layout = "randomly") +
  geom_node_point(size = 6) +
  geom_edge_link() +
  labs(title = "Random Layout")
...
#| echo: true
library(igraph)
edges <- read.csv(here::here("resources/network_data/edge_OH.csv"))
nodes <- read.csv(here::here("resources/network_data/meta_OH.csv"))
...
regular_degree <- igraph::degree(net)
...
#| echo: true
df <- read.csv(
  here::here("resources/network_data/COCAINE_MAMBO.csv"),
  row.names = 1
)
mat <- as.matrix(df)
...
ggraph(coke_net) +
  geom_edge_link(color = "gray") +
  geom_node_point(size = 8, aes(color = numb_cliques))
```
...
overlap_net <- graph_from_adjacency_matrix(
  overlap_mat,
  diag = F,
  mode = "undirected",
  weighted = T
...
ggraph(coke_net) +
  geom_edge_link(color = "gray") +
  geom_node_point(size = 8, aes(color = Cluster))
```
...
#| echo: true
net <- read_graph(
  here::here("resources/network_data/london.graphml"),
  format = "graphml"
)
net
```
...
We need to convert our distance matrices into vectors and then stick it into a dataframe. 
```{r}
#| echo: true 
data <- data.frame("Age_Dist" = c(age_dist), "SE_Dist" = c(london_se))
...
ggplot(data, aes(x = Age_Dist, y = SE_Dist)) +
  geom_jitter() +
  theme_minimal() +
  labs(y = "Struct Distance", x = "Age Distance")
```
...
#| echo: true 
#| layout-ncol: 2
plotMat(M = mat, clu = memberships, orderClu = T, mar = c(0.1, 0.1, 0.1, 0.1))
plotMat(
  M = mat > 0,
  clu = memberships,
  orderClu = T,
...
inc_df <- read.csv(
  here::here("resources/network_data/scotland.csv"),
  row.names = 1
)
...
```{r}
#| echo: true 
corp_degree <- igraph::degree(net)[V(net)$label == "Company"]
member_degree <- igraph::degree(net)[V(net)$label != "Company"]
...
We can use `which()` to identify which edges are at a certain threshold and `delete_edges()` to... delete edges 
...
pruned_net <- delete_edges(members_net, edge_to_delete)
...
diag(bonac_normalized) <- 0 ## temporay fix
bonac_memb_net <- graph_from_adjacency_matrix(
  bonac_normalized,
  diag = F,
  mode = "undirected",
  weight = T
...
If you want to get the other mode you need to transpose the matrix:

```{r}
#| echo: true 
#| cache: true
bonac_trans <- project_bonac(t(mat)) #transposed
diag(bonac_trans) <- 0
bonac_comp_net <- graph_from_adjacency_matrix(
  bonac_trans,
  diag = F,
  mode = "undirected",
  weight = T
...
plot_net <- delete_vertices(net, igraph::degree(net) == 0)
ggraph(plot_net, "kk") +
  geom_edge_link(color = "gray") +
  geom_node_point(size = 3, aes(color = dual_cluster, shape = label))
```
...
mids_df <- read.csv(here::here("resources/network_data/cow/mids_net.csv"))
nmc_data <- read.csv(here::here("resources/network_data/cow/nmc_vertex.csv"))
mids_net <- graph_from_data_frame(mids_df, vertices = nmc_data, directed = F)

trade_df <- read.csv(here::here("resources/network_data/cow/trade_net.csv"))
trade_net <- graph_from_data_frame(trade_df, vertices = nmc_data, directed = F)

cont_df <- read.csv(here::here("resources/network_data/cow/cont_net.csv"))
cont_net <- graph_from_data_frame(cont_df, vertices = nmc_data, directed = F)
```
...
```{r}
#| echo: true
mean(abs(qap$dist) >= qap$testval) # Two-sided p-value
hist(qap$dist)
```
...
- `mode=` If your network(s) are directed or undirected. 
...
mod <- netlm(
  y = dv_mat,
  x = list(same_sex, same_group, send_male, rec_male, rec_soc, send_soc),
  mode = "digraph"
)
```
...
democracy_df <- read.csv(here::here(
  "resources/network_data/cow/democracy_data.csv"
))
```
...
```{r}
#| echo: true 
all_data <- merge(trade_df, democracy_df, by.x = "name", by.y = "COWcode")
```
...
mod <- lmperm(degree ~ v2x_polyarchy + milper, data = all_data, np = 50)
```
...
- `gwesp()` captures the tendency towards transitivity. Positive values mean open triads are more likely to close.
- `gwdegree()` captures a tendency towards having equal or unequal distributions of edges. Positive values mean edges are more equally distributed, negative values mean edges tend to go to those who already have edges (popularity). 


You can set their decay with `decay=, fixed=TRUE`. 
...
I found [data of interactions](http://www.sociopatterns.org/datasets/test/) in a french workplace. It includes the department that they work in. 

```{r}
#| echo: false 
net <- read_graph(
  here::here("resources", "network_data", "french_office.graphml"),
  format = "graphml"
)
...
1. Do people in the same office interact more? `nodematch()` creates a term for if two nodes have the same attribute on "Department"
2. Are there any particular offices that interact with people more than others? `nodefactor()`: "number of times that nodes with a given level of a categorical nodal attribute appear within the edgeset"

```{.r}
set.seed(1)
mod <- ergm(asNetwork(net)~edges + nodematch("Department") + 
  nodefactor("Department") )
...
set.seed(1)
mod_simple <- ergm(
  asNetwork(net) ~ edges + nodematch("Department") + nodefactor("Department")
)
```
...
The coefficient on `nodematch.Department` is 3.53 and has a two-sided p-value < 0.01. It shows that interactions are significantly, and substantively more likely between people in the same department. For `nodefactor.Department` the DCAR department is the heldout variable. Compared to DCAR, individuals in DMCT, DMI, DSE, and SFLE are less likely to have interactions, while individuals in DISQ are more likely to have interactions. 

```{r}
#| echo: false
#| out-height: 100%
est_gof <- gof(mod_simple)
par(mfrow = c(2, 2)) # Creates a 2 by 2
plot(est_gof)
```


### What About Dyadic Dependencies? 

- Are interactions transitive? Could this be explaining homophily? We need to add `gwesp(decay=, fixed=TRUE)`
- Is the network centralized? We need to add `gwdegree(decay=, fixed=TRUE)`

We can always adjust the decay (stay between 0 and 1) and try to find one that leads to better fit (we'll get to that later). 

```{.r}
set.seed(1)
mod_simple <- ergm(asNetwork(net)~edges + nodematch("Department") + 
  nodefactor("Department") + 
...
The coefficient on homophily has decreased, so some of that effect was explained by transitvitity. The positive and significant coefficient on `gwesp.fixed.025` means that there is a tendency to close triangles, or for interactions to be transitive. In contrast the negatve coefficient on `gwdeg.fixed.025` indicates that there is a tendency towards "centralization" with interactions being more common among those who already have interactions (there is inequality in the distribution of interactions). 


```{r}
#| echo: false
#| out-height: 100%
est_gof <- gof(mod_full)
par(mfrow = c(2, 2)) # Creates a 2 by 2
plot(est_gof)
```
...
No major improvement, maybe a little worse? 
...
- `nodematch.Grupo` This shows the effect of two students being in _same_ group. This coefficient is against positive (1.41) and is significant (p-value < 0.05) meaning that students in the same group are more likely to be friends. 
...
- `gwesp.fixed.0.75` This shows how much transitivity matters to the network structure. The coefficient is positive (1.31) and statistically significant (p-value < 0.05) meaning that triangles are likely to form in this network (more than compared to a random network). In practice this indicates that transitivity happens in this networks. 
- `gwdeg.fixed.0.75` This models the distribution of degrees, the positive (1.06) and significant (p-value <0.05) means that there is a fair number of "middle value" degrees (or that friendships are more equally distributed). If it was negative we would have a tendency towards inequality in friendships (some with many friends, many with some friends).